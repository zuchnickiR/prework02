...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //Tworze tablice charów 'x' oraz 'o'
        Character[] symbol = {'x','o'};

        //Tworze tablice Stringów odnośnie wygranej
        String [] message = {"X wins", "O wins"};

        //iteruje po tablicy z symbolem x i o i dla każdego 'x' a następnie 'o' sprawdzam warunki
        for(int j=0;j<symbol.length;j++) {


            //SPRAWDZAM PRZEKĄTNE W TABLICY 2D
            ...
            //SPRAWDZAM DLA WIERSZY (DLATEGO ZACHARDKODOWAŁEM INDEXY WIERSZY) W TABLICY 2D
            for(int i=0;i<grid.length;i++) {

                if (grid[0][i] == symbol[j] && grid[1][i] == symbol[j] && grid[2][i] == symbol[j])
                    return message[j];

                //SPRAWDZAM DLA KOLUMN(DLATEGO ZACHARDKODOWAŁEM INDEXY KOLUMY) W TABLICY 2D
        ...
        /*
        podpowiedź ;)
            [WIERSZ 0][KOLUMNA 0] | [WIERSZ 0][KOLUMNA 1]  | [WIERSZ 0][KOLUMNA 2]
            [WIERSZ 1][KOLUMNA 0] | [WIERSZ 1][KOLUMNA 1]  | [WIERSZ 1][KOLUMNA 2]
            [WIERSZ 2][KOLUMNA 0] | [WIERSZ 2][KOLUMNA 1]  | [WIERSZ 2][KOLUMNA 2]
        ...
        * */
        ...
        //podwójny for (z innymi nanzwami iteratora) da mi mozliwość przeiterowania po wszystkich indexach tablicy z grą
        for(int i=0;i<grid.length;i++) {

            for(int j=0;j<grid.length;j++) {

                //jeśli pole na danym indexie jest równe '-' to do zmiennej total dodaj mi 1
        ...
        //za pętlą for, jeśli iteracja po wszystkich indexach tablicy z grą wyniesie 9, które przyrównam do iloczynu rzedów i kolumn to
        //zwróć Stringa remis w przeciwnym razie zwróć Stringa z rezultatem powyższych if'ów
